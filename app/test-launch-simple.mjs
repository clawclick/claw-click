...
// LaunchBundler ABI (launchAndMint function)
...
async function main() {
  ...
  const launchParams = {
    ...
    launchType: 0, // 0 = AGENT (claw.click), 1 = DIRECT (claws.fun)
  }
  
  // Total ETH = 0.001 bootstrap + 0.005 immortalization fee
  const totalETH = parseEther('0.006')
  ...
  try {
    const { request, result } = await publicClient.simulateContract({
      ...
      args: [
        ...
        'ClawdeBot', // agentName
        '@ClawdeBot', // socialHandle
        '', // memoryCID (empty for now)
        '', // avatarCID (empty for now)
        '', // ensName (empty for now)
      ],
      value: totalETH,
      account,
    })
    
    const [tokenAddress, poolId, nftId] = result
    console.log('   Simulation successful!')
    console.log('   Expected Token:', tokenAddress)
    console.log('   Expected NFT ID:', nftId.toString())
