...
contract ClawclickFactory is Ownable, ReentrancyGuard {
    ...
    /// @notice Launch type: DIRECT (claws.fun) or AGENT (claw.click)
    ...
    struct LaunchInfo {
        ...
        LaunchType launchType;    // DIRECT or AGENT
    }
        ...
        returns (address token, PoolId poolId) 
    {
        ...
        // 5. Register launch with hook (AGENT only)
    ...
    function _createPoolKey(address token, LaunchType launchType) internal view returns (PoolKey memory) {
        ...
        if (launchType == LaunchType.DIRECT) {
            ...
            // AGENT (claw.click): Hook with dynamic fee
            return PoolKey({
                currency0: currency0,
                currency1: currency1,
