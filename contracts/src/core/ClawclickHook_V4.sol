...
contract ClawclickHook is BaseHook, ReentrancyGuard {
    ...
    function beforeSwap(
        ...
        IClawclickFactory.LaunchInfo memory launchInfo = factory.getLaunchByPoolId(poolId);
        ...
        // If creator first-buy window active, bypass restrictions
        if (isCreatorFirstBuy || activationInProgress[poolId]) {
        ...
        if (feeAmount > 0) {
            ...
            IClawclickFactory.LaunchInfo memory factoryLaunchInfo = IClawclickFactory(config.factory()).getLaunchByPoolId(poolId);
    ...
    /**
     * @notice Get maxTx limit based on growth ratio from starting MCAP
     * @dev Starting limit is determined by startMcap (1e=0.1%, 5e=0.5%, 10e=1.0%)
     *      Then scales proportionally with growth: limit = startingLimit * (currentMcap / startMcap)
     * @param currentMcap Current MCAP in wei
     * @param startMcap Starting MCAP in wei
     * @return maxTx Maximum transaction amount
     */
    function _getMaxTx(uint256 currentMcap, uint256 startMcap) internal view returns (uint256 maxTx) {
        // Get starting limit from Config based on chosen starting MCAP
        // 1e=0.1%, 2e=0.2%, ..., 10e=1.0%
        uint256 baseLimitBps = config.getStartingLimit(startMcap);
        
        // Calculate growth ratio (scaled by BPS for precision)
        uint256 growthRatio = (currentMcap * BPS) / startMcap;
        
        // Scale limit by growth ratio
        uint256 limitBps = (baseLimitBps * growthRatio) / BPS;
        
        // Minimum is the starting limit
        if (limitBps < baseLimitBps) {
            limitBps = baseLimitBps;
        }
    ...
    /**
     * @notice Get maxWallet limit based on growth ratio from starting MCAP
     * @dev Same calculation as maxTx (growth-based scaling from starting limit)
     * @param currentMcap Current MCAP in wei
     * @param startMcap Starting MCAP in wei
     * @return maxWallet Maximum wallet balance
     */
    function _getMaxWallet(uint256 currentMcap, uint256 startMcap) internal view returns (uint256 maxWallet) {
